...
class UVit2DConvEmbed(nn.Module):
    ...
    def forward(self, input_ids):
        embeddings = self.embeddings(input_ids)     # [B, H, W, D]
        embeddings = self.layer_norm(embeddings)
        embeddings = embeddings.permute(0, 3, 1, 2)     # [B, D, H, W]
...
class Transformer2DModel(ModelMixin, ConfigMixin, PeftAdapterMixin, FromOriginalModelMixin):
    ...
    _supports_gradient_checkpointing = True #True 
    ...
    def forward(
        ...
        if noise_emb_perturb > 0:  # perturb the token embeddings with noise
            noise = torch.randn_like(hidden_states)
            hidden_states = noise_emb_perturb * noise + np.sqrt(1-noise_emb_perturb**2) * hidden_states

        encoder_hidden_states = self.context_embedder(encoder_hidden_states) 
        encoder_hidden_states = self.encoder_proj_layer_norm(encoder_hidden_states)
        hidden_states = self.down_block(hidden_states)      # from [64, 64] to [32, 32]
