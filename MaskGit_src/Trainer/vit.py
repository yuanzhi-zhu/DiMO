...
class MaskGIT(Trainer):
    ...
    def train_one_epoch(self, log_iter=2500):
        ...
        for x, y in bar:
            ...
            with torch.cuda.amp.autocast():                             # half precision
                pred = self.vit(masked_code, y, drop_label=drop_label)  # The unmasked tokens prediction: logits
                # Cross-entropy loss
    ...
    def cfg_pred_model(self, model, code, label, cfg):
        drop = torch.ones(code.shape[0], dtype=torch.bool).to(self.args.device)
        if cfg != 0:
            ...
            logit = (1 + cfg) * logit_c - cfg * logit_u
        else:
            logit = model(code, label, drop_label=~drop)
        return logit
    ...
    def sample(self, init_code=None, nb_sample=50, labels=None, sm_temp=1, w=3,
               randomize="linear", r_temp=4.5, sched_mode="arccos", step=12, model=None):
        ...
        """
        model = self.vit if model is None else model
        model.eval()
        ...
        with torch.no_grad():
            ...
            for indice, t in enumerate(scheduler):
                ...
                with torch.cuda.amp.autocast():  # half precision
                    logit = self.cfg_pred_model(model, code, labels, w * (indice / (len(scheduler)-1)))
                ...
                l_codes.append(pred_code.view(nb_sample, self.patch_size, self.patch_size).clone())     # pred_x0
    ...
    @torch.no_grad()
    def sample_one_step(self, nb_sample=10, labels=None, masked_code=None):
        self.vit.eval()
        if labels is None:  # Default classes generated
            # goldfish, chicken, tiger cat, hourglass, ship, dog, race car, airliner, teddy bear, random
            labels = [1, 7, 282, 604, 724, 179, 751, 404, 850, random.randint(0, 999)] * (nb_sample // 10)
            labels = torch.LongTensor(labels).to(self.args.device)
        _, code, _, _ = self.generate_one_step(nb_sample, labels, masked_code)
        # decode the final prediction
        _code = torch.clamp(code, 0,  self.codebook_size-1)
        x = self.ae.decode_code(_code)
        self.vit.train()
