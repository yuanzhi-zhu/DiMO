...
class MaskTransformer(nn.Module):
    def __init__(self, img_size=256, hidden_dim=768, codebook_size=1024, depth=24, heads=8, mlp_dim=3072, dropout=0.1, nclass=1000):
        ...
        self.num_visual_tokens = self.patch_size * self.patch_size
        ...
        self.dtype = self.bias.dtype

    def forward(self, img_token, y=None, drop_label=None, return_attn=False, noise_emb_perturb=0):
        """ Forward.
            :param:
        ...
        if drop_label is not None:
            ...
            # Handle the case where drop_label is None
        ...
        if noise_emb_perturb > 0:  # perturb the visual token embeddings with noise
            noise = torch.randn_like(tok_embeddings)[:, :self.num_visual_tokens, :]
            tok_embeddings[:, :self.num_visual_tokens, :] = noise_emb_perturb * noise + np.sqrt(1-noise_emb_perturb**2) * tok_embeddings[:, :self.num_visual_tokens, :]
        # Position embedding
        ...
        if return_attn:  # return list of attention
            return logit[:, :self.num_visual_tokens, :self.codebook_size + 1], attn

        return logit[:, :self.patch_size*self.patch_size, :self.codebook_size+1]


    def save_pretrained(self, save_directory, filename="pytorch_model.bin"):
    ...
    @classmethod
    def from_pretrained(self, save_directory, filename="pytorch_model.bin", map_location=None, strict=True, **kwargs):
        # Load the state_dict from the file
        load_path = os.path.join(save_directory, filename)
        ...
        print(f"Missing keys: {missing_keys}") if len(missing_keys) > 0 else None
